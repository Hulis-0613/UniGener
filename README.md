...
- `ast`: 仅使用仓库 AST 索引检索。
- `lsp`: 仅使用 LSP 适配层检索（Python 使用 Jedi；Java 使用定义跳转式符号定位）。
- `hybrid`: 同时使用 AST + LSP，并自动去重。
- Java LSP 可通过 `UNIGENER_JDTLS_CMD` 指定 `jdtls` 启动命令；未配置或不可用时自动回退到内置定义定位实现。
...
python -m examples.generate_java_repo_tests \
  ...
  --llm-provider openai \
  --retrieval-mode ast \
  --concurrent-generate-workers 4 \
  --sandbox-workers 4 \
  --session-id sprint_20260401 \
  --auto-run-tests \
...
- `logs/<repo>/<session_id>/agents/agent_api_calls.jsonl`：模型 API 调用日志（含 `session_id`、prompt/response、重试次数、错误信息）。
- `logs/<repo>/<session_id>/agents/intent_planning.jsonl`：意图规划日志（含 `session_id`、原始 JSON、解析结果、queries）。
- `logs/<repo>/<session_id>/agents/code_retrieval.jsonl`：代码检索日志（含 `session_id`、`java_lsp_backend`、`lsp_reason_stats`）。
- `logs/<repo>/<session_id>/sandbox_iterations/*.json`：Java 迭代沙箱每轮结果（编译/运行/覆盖率/错误栈）。
- `logs/<repo>/<session_id>/sandbox_batch/*.json`：并发 sandbox 批量结果（按 worker 文件命名）。
- `logs/<repo>/<session_id>/pipeline/repo_concurrency_tasks.jsonl`：同 repo 并发任务日志（`task_id`、阶段、耗时、queue wait、selector）。
- `--results-output-json`：批量生成脚本的聚合统计结果（总数、编译错误占比、运行错误占比、可运行占比、平均覆盖率、平均生成排队耗时、平均生成耗时、平均沙箱排队耗时、平均沙箱耗时、P50/P95 时延）。
...
- logs 目录按 `<repo>/<session_id>/` 分层保存日志，便于按仓库与会话快速排查。
